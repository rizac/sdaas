'''
Tests the snippets providede in the README
...
#         file = join(dataroot, 'trace_GE.APE.mseed')
#         inv = join(dataroot, 'inventory_GE.APE.xml')
#         ret = readme_snippet(file, inv)
#         print()
#         print('SNIPPET for GITHUB README (copy and paste into README if '
#               'not already there):')
#         print()
#         print(ret)


def makesnippet(title, code, locals, eval_last_line=True):
    code_lines = code.strip().split('\n')
    if eval_last_line:
        code = "\n".join(code_lines[:-1]) + '\n__var__= (' + code_lines[-1] + ")"
    exec(code, globals(), locals)
    code_str = '\n'.join(f'>>> {_}' for _ in code_lines)
    result = repr(locals['__var__']) + '\n' if eval_last_line else ''
...
def create_readme_snippet():
    ...
    content = ''
#     content = '```python\n'
#     #content = '|Function|easy import|source (in the borwser, try to click for details)|\n'
#     #content += '| --- | --- | --- |\n'
#     import sdaas.core
#     for funcname in dir(sdaas.core):
#         if funcname[:1] == '_':
#             continue
#         try:
#             func = getattr(sdaas.core, funcname)
#             mod = func.__module__
#         except AttributeError:
#             continue
#         content += f'>>> from {mod} import {funcname}  # EASY IMPORT: from from sdaas.core import {funcname}\n'
#     content += '```\n'
