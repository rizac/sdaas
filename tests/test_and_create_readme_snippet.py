...
def get_code_snippet_bounds(readme_content, start_index):
    ...
    ```)"""
    tag1 = "<!--\nWARNING: do not edit here the code snippet, it's autogenerated "
    idx1 = readme_content[start_index:].index(tag1)
    tag2 = "```"
    idx2 = readme_content[idx1:].find(tag2)
    idx3 = readme_content[idx2:].find(tag2)
    return idx1 + idx2, idx1 + idx3 + len(tag2)
...
def wrap_in_code_snippet(code_text, code_output=None):
    if not code_output:
        code_output = ''
    return f'''```python
    {code_text}
