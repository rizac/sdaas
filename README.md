...
<!-- IMPORTANT: EACH "```python" code snippet is executed in `test_and_create_code_snippet`
to check that it works. Also, its output, if any, is INSERTED BELOW THE SNIPPET (with
a comment tag to mark the snippet as auto-generated). THIS MEANS THAT RUNNING TESTS MIGHT
NEED A COMMIT OF THE README file -->
...
# mock a list of streams by loading twice the same Stream:
